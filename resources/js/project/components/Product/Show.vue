<template>
	<section v-if="product" class="content">
		<div class="content__container">
			<h2 class="_title mx-body">
				Информация о товаре {{ product.title }}
			</h2>
			<div class="content__body">
				<div class="moreinfo-tovar__info moreinfo-tovar-info">
					...
					<div class="moreinfo-tovar-info__textinfo moreinfo-tovar-info-textinfo">
						<ul class="moreinfo-tovar-info-textinfo__categories moreinfo-tovar-info-textinfo-categories">
							<li class="moreinfo-tovar-info-textinfo-categories__list">
								<span class="moreinfo-tovar-info-textinfo-categories__title">Категория</span>
							</li>
						...
						<div class="moreinfo-tovar-info-textinfo__title">
							<span class="code">Код: {{ product.id }}</span> {{ product.title }}
						</div>
						<div class="moreinfo-tovar-info-textinfo__price moreinfo-tovar-info-textinfo-price">
							<div v-if="product.price_old > 0" class="moreinfo-tovar-info-textinfo-price__old moreinfo-tovar-info-textinfo-price-old">
							...
							<div class="moreinfo-tovar-info-textinfo-price__new moreinfo-tovar-info-textinfo-price-new">
								<span class="moreinfo-tovar-info-textinfo-price-new__number">
									{{ product.price_now }}
								</span>
						...
						<div class="moreinfo-tovar-info-textinfo__ratinginfo moreinfo-tovar-info-textinfo-ratinginfo">
							...
							<a href data-goto-header data-goto=".moreinfo-tovar-body-reviews__form" class="moreinfo-tovar-info-textinfo-ratinginfo__column moreinfo-tovar-info-textinfo-ratinginfo__writereview moreinfo-tovar-info-textinfo-ratinginfo-writereview">
				...
				<div class="moreinfo-tovar__content">
					...
					<div class="moreinfo-tovar-body">
						<div class="moreinfo-tovar-body__specifications moreinfo-tovar-body-specifications">
							<div class="moreinfo-tovar-body-specifications__title">Характеристика</div>
							<div class="moreinfo-tovar-body-specifications__content">
								<li v-for="specification in product.specifications" :key="specification" class="moreinfo-tovar-body-specifications__list moreinfo-tovar-body-specifications-list">
									<strong class="moreinfo-tovar-body-specifications-list__title">{{ specification.name }}: </strong>
									<span v-for="item in specification.data" :key="item" class="moreinfo-tovar-body-specifications-list__text">{{ item.name }}</span>
								</li>
							</div>
						</div>
						<div v-if="description" class="moreinfo-tovar-body__description moreinfo-tovar-body-description">
							<div class="moreinfo-tovar-body-description__title">Описание</div>
							<div class="moreinfo-tovar-body-description" v-html="description">
							</div>
						</div>
	...
	<commentary-popup v-show="product"></commentary-popup>
	<histories-block :status="status"></histories-block>
	<rec-block :status="status" :id="product && product.categories && product.categories.length > 0 ? product.categories[product.categories.length - 1].id : 0"></rec-block>
</template>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			this.$store.dispatch('setSaveTovarHistory', this.$route.params.id)
			this.getProduct();
			this.$store.dispatch('zeroingCommentaries');
			this.$store.commit('setApiSrcCommentaries', 'commentaries/' + this.$route.params.id);
			this.$store.commit('setMessage', 'В вашей корзине пока нет сохраненных товаров!');
			this.$store.dispatch('initializeCommentaries');
			popup;
		},
		methods: {
			async getProduct() {
