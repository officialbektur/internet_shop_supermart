<template>
	<div class="product__block product-block">
		<router-link :to="{ name: 'show', params: {id: product.id} }" class="product-block__image product-block-image">
			<div class="product-block-image__content">
				<div v-if="product.discount == 1 || product.hit == 1" class="product-block-image__sticker product-block-image-sticker">
					<div class="product-block-image-sticker__content">
						<div v-if="product.discount == 1" class="product-block-image-sticker__item product-block-image-sticker__discount">
							Скидка
						</div>
						<div v-if="product.hit == 1" class="product-block-image-sticker__item product-block-image-sticker__hit">
							Хит
						</div>
					</div>
				</div>
						...
						<div class="product-block-image-item__img">
							...
							<div v-if="product.images.length - (index + 1) == 0 && product.images_count - (index + 1) > 0" class="product-block-image-item__infoimagetext product-block-image-item-infoimagetext">
								...
								<div class="product-block-image-item-infoimagetext__content">
									Ещё <span class="product-block-image-item-infoimagetext__number">{{ product.images_count - (index + 1) }}</span> фото
								</div>
							</div>
						</div>
		...
		<div class="product-block__info product-block-info">
			...
			<div class="product-block-info__price product-block-info-price">
				<div v-if="product.price_old > 0" class="product-block-info-price__old product-block-info-price-old">
			...
			<div class="product-block-info__ratinginfo product-block-info-ratinginfo">
				<router-link :to="{ name: 'show', params: {id: product.id}, hash: '#moreinfo-tovar__content' }" class="product-block-info-ratinginfo__column product-block-info-ratinginfo__rating product-block-info-ratinginfo-rating">
				...
				<router-link :to="{ name: 'show', params: {id: product.id}, hash: '#moreinfo-tovar-body-reviews__form' }" class="product-block-info-ratinginfo__column product-block-info-ratinginfo__writereview product-block-info-ratinginfo-writereview">
			...
			<div class="product-block-info__buy">
				<a :href="'/' + product.id" class="product-block-info-buy__link product-block-info-buy__link  product-block-info-buy-link">
