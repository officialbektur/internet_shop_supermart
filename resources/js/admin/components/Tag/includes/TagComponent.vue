<template>
	<tr
		...
		<td class="block-table__td block-table__tdedit">
			<router-link :to="{ name: 'tags.update', params: { id: tag.id ?? 0 } }" class="block-table__edit block-table-edit">
			...
			async deleteTag(id, index) {
				...
				try {
					...
					if (response && response.data) {
						const updatedTags = this.$store.getters.tags;
						updatedTags[index].status = response.data.status;
						tags = updatedTags;
						isReadOnly = false;
					} else {
						isReadOnly = false;
