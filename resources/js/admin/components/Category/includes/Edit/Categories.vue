<template>
	<router-link :to="{ name: 'categories.update', params: { id: category.id } }" v-if="!category.children" class="block-list">
	...
	<div v-else class="block-sublist">
		...
		<div hidden class="block-sublist__menu">
			<router-link :to="{ name: 'categories.update', params: { id: category.id } }" class="block-list__default block-list-default block-list">
				<div class="block-list__name block-list-default__title">
					Изменить эту категорию: {{ category.name }}
				</div>
			</router-link>
