...
export function minWindowScreen() {
...
export function ibg() {
	let ibg = document.querySelectorAll("[data-ibg]");
...
// Полифил ДЛЯ ~Closest~
export function polyfillSupportClosest() {
...
export let _slideUp = function (target, duration = 500, showmore = 0) {
...
export let _slideDown = function (target, duration = 500, showmore = 0) {
	if (!target.classList.contains('_slide')) {
		target.classList.add('_slide');
...
export let _slideToggle = function (target, duration = 500) {
	if (target.hidden) {
...
export let bodyLockToggle = function () {
	let delay = 500;
...
export let bodyUnlock = function () {
	let delay = 500;
		...
		function initSpollerBody(spollersBlock, hideSpollerBody = true) {
			...
			if (spollerTitles.length) {
				spollerTitles = Array.from(spollerTitles).filter( function (item) {
					if (item.closest('[data-spollers]') === spollersBlock) {
						return item;
					}
				});
...
export function menuInit() {
	if (document.querySelector(".icon-menu")) {
		...
		let content = document.querySelectorAll(".sidebar-sublist__content");

		iconMenu.addEventListener('click', function (e) {
			if (bodyLockStatus && e.target.closest('.icon-menu')) {
				bodyLockToggle();
				document.documentElement.classList.toggle("menu-open");
				if (emSreenWidth(991.98)) {
	...
	window.addEventListener("load", function (e) {
		...
		function initItem(showMoreBlock, matchMedia = false) {
			...
			showMoreContent = Array.from(showMoreContent).filter( function (item) {
				if (item.closest('[data-showmore]') === showMoreBlock[0]) {
					return item;
				}
			});
...
/* ===================================  Progress Bar  --End--  =================================== */
export function emSreenWidth(sreenWidth = 767.98) {
	let numberDefault = Number(window.getComputedStyle(document.documentElement).getPropertyValue('font-size').match(/\d+/)[0]);
