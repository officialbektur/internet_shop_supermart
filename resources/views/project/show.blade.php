...
@section('title')Информация о {{ $product->title }}@endsection
@section('content')
			...
			<div class="moreinfo-tovar__info moreinfo-tovar-info">
				<div class="moreinfo-tovar-info__image moreinfo-tovar-info-image">
					<div class="moreinfo-tovar-info-image__sliderbig moreinfo-tovar-info-image-sliderbig">
						@if($product->discount == 1 || $product->hit == 1)
							<div class="moreinfo-tovar-info-image-sliderbig__sticker moreinfo-tovar-info-image-sliderbig-sticker">
								<div class="moreinfo-tovar-info-image-sliderbig-sticker__content">
									@if($product->discount == 1)
										<div class="moreinfo-tovar-info-image-sliderbig-sticker__discount">
											Скидка
										</div>
									@endif
									@if($product->hit == 1)
										<div class="moreinfo-tovar-info-image-sliderbig-sticker__hit">
											Хит
										</div>
									@endif
								</div>
						...
						<div data-gallery class="swiper-wrapper">
							@foreach($product->media as $index => $media)
								@if($media->type == "video")
									<a data-video='{"source":[{"src":"{{ $media->video }}","type": "video/mp4"}],"attributes":{"preload": false,"controls": true}}' data-hash="image_{{ $index }}" class="swiper-slide moreinfo-tovar-info-image-sliderbig__slide moreinfo-tovar-info-image-sliderbig-slide">
										<span class="moreinfo-tovar-info-image-sliderbig-slide__image">
											<video class="myVideo" src="{{ $media->video }}" controls played muted loop></video>
										</span>
										<span class="moreinfo-tovar-info-image-sliderbig-slide__background">
											<img src="{{ $media->src_max }}" alt="{{ $product->title }}_{{ $product->id }}" />
										</span>
									</a>
								@else
									<a data-src="{{ $media->src_max }}" data-hash="image_{{ $index }}" class="swiper-slide moreinfo-tovar-info-image-sliderbig__slide moreinfo-tovar-info-image-sliderbig-slide">
										<span class="moreinfo-tovar-info-image-sliderbig-slide__image">
											<img src="{{ $media->src_max }}" alt="{{ $product->title }}_{{ $product->id }}" />
										</span>
										<span class="moreinfo-tovar-info-image-sliderbig-slide__background">
						...
						<div class="swiper-wrapper">
							@foreach($product->media as $media)
								<div class="swiper-slide">
									<img src="{{ $media->src_min }}" loading="lazy" alt="{{ $product->title }}_{{ $product->id }}" />
				...
				<div class="moreinfo-tovar-info__textinfo moreinfo-tovar-info-textinfo">
					<ul class="moreinfo-tovar-info-textinfo__categories moreinfo-tovar-info-textinfo-categories">
						...
						@foreach($product->categories as $category)
							<li class="moreinfo-tovar-info-textinfo-categories__list">
								...
								<a href="/categories/{{$category['id']}}" class="moreinfo-tovar-info-textinfo-categories__name">{{$category['name']}}</a>
							</li>
						@endforeach
					</ul>
					...
					<div class="moreinfo-tovar-info-textinfo__price moreinfo-tovar-info-textinfo-price">
						@if($product->price_old > 0)
							<div class="moreinfo-tovar-info-textinfo-price__old moreinfo-tovar-info-textinfo-price-old">
			...
			<div class="moreinfo-tovar__content">
				...
				<div class="moreinfo-tovar-body">
					<div class="moreinfo-tovar-body__description moreinfo-tovar-body-description">
						{{ isset($description->desc) ? $description->desc : '' }}
					</div>
					<commentary></commentary>
				</div>
			</div>
		</div>
		@include("project.includes.blocks.more-loading")
	</div>
</section>
@endsection